package com.xtensolutions.core.model
...
sealed class ListItem(open val data: Any?) {
    ...
    /** A header/section item. Implements [ListItem] so adapters that rely on
     * that interface can detect headers easily. */
    data class Section<T>(override val data: T) : ListItem(data)
